...
// const EARARTree = require( path.join(__dirname, 'myarar.js') );
...
$('button[id="inspect-button"]').attr('type', 'button').on('click', function () {
	...
	function startToInspect () {
		...
		Object.keys(myObject['aclObject']).forEach(function ( nodeName, nodeNameCount ) {
			...
			// console.log( util.inspect(curNode.ruleList, { showHidden: false, depth: null }) ); 
			if ( curNode['ruleList'].length === 0 ) return;
			let ruleCount = 0, flagRuleCount = 0;

			// if ( nodeNameCount === 0 ) {
				console.log(curNode['ruleList']);

			curNode['ARARTree'] = new EARARTree(curNode['ruleList'], myObject['inspInfo']['segmentMode'], true);
			console.log(curNode['ARARTree']);
			// console.log(curNode['ruleList'][0]);
			// curNode['ARARTree'] = new EARARTree(curNode['ruleList'],
			// 	function ( rule ) {
			// 		return {
			// 			listOrder: rule['listOrder'],
			// 			min_sip: (new AddressPrefixObject(rule['src_ip']))['ipMinNumber'],
			// 			max_sip: (new AddressPrefixObject(rule['src_ip']))['ipMaxNumber'],
			// 			min_dip: (new AddressPrefixObject(rule['dest_ip']))['ipMinNumber'],
			// 			max_dip: (new AddressPrefixObject(rule['dest_ip']))['ipMaxNumber']
			// 		}
			// 	});
			// console.log(curNode['ARARTree']);
...
function createAnomalyChart ( event ) {
	...
	let $porttab = `<li id="li-block${this.index}">
						<a data-toggle="tab"  href="#tab-${this.index}">block${this.index}</a>
					</li>`;	
	let $chart = fs.readFileSync(`${__dirname}/templates/block-information.html`, 'utf-8').toString();
	// console.log($chart);
	// $($porttab).appendTo('#portchart-tabs');
...
function depictResult () {
	...
	Object.keys(myObject['aclObject']).forEach(function ( nodeName, nodeNameCount ) {
		...
		let $tab = `<li id="li-${nodeName}">
						<a data-toggle="tab"  href="#tab-${nodeName}">${nodeName}</a>
					</li>`;
		...
		let $chart = `<div id="tab-${nodeName}" class="tab-pane fade " >
						...
						<div class="row"> 
							<div class="col-xs-12" id="block-content">
							</div> 
						</div>
					</div>`;
	...
	function createSeries ( dataList ) {
		let seriesList = [];
		// console.log( util.inspect(dataList, { showHidden: false, depth: null }) );
...
// function forexample(event)
// {
// 	console.log(event);
	
// 		createAnomalyChart.apply(event);
// }
