...
function ARARTree ( inputData, segmentMode=false, useExchange=true, initialLevel=1 ,ordertype=false ) {
	...
	// console.log( util.inspect(exchangedInputData, { showHidden: false, depth: null }) );
	...
	while ( true ) {
		...
		if ( node ) {
			...
			if ( (childSum === 0) && (node['ruleList'].length > 0) ) {
				...
				// let weight=(node['ruleList'][0]['max_sip'] -node['ruleList'][0]['min_sip'] )*(node['ruleList'][0]['max_dip']-node['ruleList'][0]['min_dip']);
				// console.log(weight);
				node['region'] = {
					'max_sip' : node['ruleList'][0]['max_sip'],
					'min_sip' : node['ruleList'][0]['min_sip'],
					'max_dip' : node['ruleList'][0]['max_dip'],
					'min_dip' : node['ruleList'][0]['min_dip'],
					'weight':(node['ruleList'][0]['max_sip'] -node['ruleList'][0]['min_sip']+1 )*(node['ruleList'][0]['max_dip']-node['ruleList'][0]['min_dip']+1),
				};
				// console.log(node);
				node['ruleList'].forEach(function ( data ) {
					...
					let newData;
					if ( useExchange ) {
						// console.log(useExchange);
						if ( data['isExchange'] ){
							if(ordertype){
								newData = exchangedInputData[data['ruleOrder']];
							}
							else{
							...
							if(ordertype){
								newData = inputData[data['ruleOrder']];
							}
							else{
 ...
 *	@ parameter
 */
function ARARRule ( listOrder, min_sip, max_sip, min_dip, max_dip, flag, isExchange, ruleOrder ) {
	...
	node['ruleList'].forEach(function ( curRule ) {
		...
		switch ( ruleLocate.join('') ) {
			 ...
			 */
				...
				newRule = new ARARRule(curRule['listOrder'], curRule['min_sip'], (rsvSrcReal - 1), curRule['min_dip'], curRule['max_dip'], curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node00'] === undefined )
					node['node00'] = new ARARNode(segmentMode);
				if ( segmentMode )
				...
				newRule = new ARARRule(curRule['listOrder'], rsvSrcReal, curRule['max_sip'], curRule['min_dip'], curRule['max_dip'], curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node10'] === undefined )
					node['node10'] = new ARARNode(segmentMode);
				if ( segmentMode )
					if ( newRule['flag'] === true )
			 ...
			 *  -----------
			 */
				newRule = new ARARRule(curRule['listOrder'], curRule['min_sip'], curRule['max_sip'], curRule['min_dip'], (rsvDestReal - 1), curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node00'] === undefined )
					node['node00'] = new ARARNode(segmentMode);
				if ( segmentMode )
					if ( newRule['flag'] === true )
				...
				newRule = new ARARRule(curRule['listOrder'], curRule['min_sip'], curRule['max_sip'], rsvDestReal, curRule['max_dip'], curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node01'] === undefined )
					node['node01'] = new ARARNode(segmentMode);
				if ( segmentMode )
					if ( newRule['flag'] === true )
			 ...
			 *  -----------
			 */
			 	newRule = new ARARRule(curRule['listOrder'], curRule['min_sip'], (rsvSrcReal - 1), curRule['min_dip'], (rsvDestReal - 1), curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node00'] === undefined )
					node['node00'] = new ARARNode(segmentMode);
				if ( segmentMode )
					if ( newRule['flag'] === true )
				...
				newRule = new ARARRule(curRule['listOrder'], curRule['min_sip'], (rsvSrcReal - 1), rsvDestReal, curRule['max_dip'], curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node01'] === undefined )
					node['node01'] = new ARARNode(segmentMode);
				if ( segmentMode )
					if ( newRule['flag'] === true )
				...
				newRule = new ARARRule(curRule['listOrder'], rsvSrcReal, curRule['max_sip'], curRule['min_dip'], (rsvDestReal - 1), curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node10'] === undefined )
					node['node10'] = new ARARNode(segmentMode);
				if ( segmentMode )
					if ( newRule['flag'] === true )
				...
				newRule = new ARARRule(curRule['listOrder'], rsvSrcReal, curRule['max_sip'], rsvDestReal, curRule['max_dip'], curRule['flag'], curRule['isExchange'], curRule['ruleOrder']);
				if ( node['node11'] === undefined )
					node['node11'] = new ARARNode(segmentMode);
				if ( segmentMode )
					if ( newRule['flag'] === true )
...
function inputDataConvertor ( dataList ) {
	...
	for (let dataCount=0; dataCount<dataList.length; dataCount++) {
		...
		newData = new ARARRule(data['listOrder'], src_ip['ipMinNumber'], src_ip['ipMaxNumber'], dest_ip['ipMinNumber'], dest_ip['ipMaxNumber'], flag, data['isExchange'], data['ruleOrder']);
		newDataList.push(newData);
	}
	return newDataList;
}
